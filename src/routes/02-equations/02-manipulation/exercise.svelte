...
<script lang="ts">
	...
	const title = 'Manipulation of Equations';
	const nextSectionName = 'Solving Linear Equations';
	const nextSectionSlug = '../03-solving-linear-equations/example';

	// qn props
	export let variables: [number, number, number, boolean, boolean, boolean, boolean, boolean];
	...
	function newQn(): void {
		...
		// update qn
		({ qn, answer, working } = generateQn(...generateNewVariables(), level));
		// reset qn
